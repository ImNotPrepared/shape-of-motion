...
class SceneModel(nn.Module):
    ...
    def render_tmp_int(
        ...
        if means is None or quats is None:
            means, quats = pose_fnc(
                torch.tensor([t[0]], device=device) if t is not None else None
            )
            means = means[:, 0]
            quats = quats[:, 0]
        ...
        ### new t
        means_new_t, quats_new_t = pose_fnc(
            torch.tensor([t[1]], device=device) if t is not None else None
        )
        means_new_t = means_new_t[:, 0]
        quats_new_t = quats_new_t[:, 0]
        means = (means_new_t + means) / 2 
        quats = (quats_new_t + quats) / 2
